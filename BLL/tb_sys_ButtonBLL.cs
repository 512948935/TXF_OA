...
namespace BLL
{
    public class tb_sys_ButtonBLL : BaseBLL<tb_sys_Button>, Itb_sys_ButtonBLL
    {
        #region 接口注入反转
        /// <summary>
        /// 该接口负责用户自定义的功能实现
        /// </summary>
        private Itb_sys_ButtonDAL myDao;
        /// <summary>
        /// 构造函数(接口转换,Dao只负责基类的增删改查)
        /// </summary>
        public tb_sys_ButtonBLL(Itb_sys_ButtonDAL dao)
        {
            Dao = myDao = dao;
        }
        #endregion
        ...
        public DataTable GetPageList(bool fromCache, int page, int pagesize, out int total, string code, string disabled, List<WhereField> listWhere)
        {
            ...
            {
                cacheKey = string.Format("page:{0},pagesiez:{1},code:'{2}',disabled:'{3}',name:'{4}'", page, pagesize, code, disabled, "tb_sys_Button");
