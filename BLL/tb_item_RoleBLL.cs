...
namespace BLL
{
    public class tb_item_RoleBLL : BaseBLL<tb_item_Role>, Itb_item_RoleBLL
    {
        #region 接口注入反转
        /// <summary>
        /// 该接口负责用户自定义的功能实现
        /// </summary>
        private Itb_item_RoleDAL myDao;
        /// <summary>
        /// 构造函数(接口转换,Dao只负责基类的增删改查)
        /// </summary>
        public tb_item_RoleBLL(Itb_item_RoleDAL dao)
        {
            Dao = myDao = dao;
        }
        #endregion
        ...
        public DataTable GetPageList(bool fromCache, int page, int pagesize, out int total, string code, string disabled, List<WhereField> listWhere)
        {
            ...
            {
                cacheKey = string.Format("page:{0},pagesiez:{1},code:'{2}',disabled:'{3}',name:'{4}'", page, pagesize, code, disabled, "tb_item_Role");
