function currentDate() {
    ...
    return y + (m < 10 ? ('0' + m) : m) + (d < 10 ? ('0' + d) : d) + (hh < 10 ? ('0' + hh) : hh) + (mm < 10 ? ('0' + mm) : mm) + (ss < 100 ? ('0' + ss) : ss)
}
//定义一个区域图类：
function GooFlow(bgDiv, property) {
    ...
    if (property.haveHead) {
        tmp = "<div class='GooFlow_head'><label title='" + (property.initLabelText || "流程绘制") + "'>" + (property.initLabelText || "流程绘制") + "</label>";
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            //e.data.inthis.$max++;
            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + currentDate(), { name: "node_" + currentDate(), left: X, top: Y, type: e.data.inthis.$nowType });
        });
        //划线时用的绑定
...
GooFlow.prototype = {
    ...
    initGroup: function (width, height) {
        ...
        this.$group.on("click", { inthis: this }, function (e) {
            ...
            //TXF 2015-01-13
            e.data.inthis.addArea(e.data.inthis.$id + "_area_" + currentDate(), { name: "area_" + currentDate(), left: X, top: Y, color: color[e.data.inthis.$max % 4], width: 200, height: 100 });
            //e.data.inthis.$max++;
            return false;
        });
    },
    ...
    addNode: function (id, json) {
        ...
        if (json.type != "start" && json.type != "end") {
            json.width = json.width ? json.width : 120;
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
            ...
            if (lineStart) This.addLine(This.$id + "_line_" + currentDate(), { from: lineStart.id, to: this.id, name: "" });
            //This.$max++;
        });
        //绑定双击编辑事件
