...
namespace Dao
{
    public class tb_item_CompanyDAL : BaseDAL<tb_item_Company>, Itb_item_CompanyDAL
    {
            ...
            try
            {
                ...
                string sql = string.Format(@"SELECT  COUNT(1) CNT 
                                                FROM (SELECT a.ID
                                                            ...
                                                            ,SetUpTime
                                                            ,a.IsDisabled
                                                      FROM   dbo.tb_item_Company a WHERE a.Marks=1
                                                    ) t WHERE {0} ", where);
        ...
        public void DeleteDepinfo(string id)
        {
            try
            {
                string sql = "UPDATE dbo.tb_item_Company SET Marks=0 WHERE ID IN " + id + "";
